	...
	match data.message:
		Message.updateUserAttributes:
			...
			if data.has("data") and sender_id in clients:
				...
				broadcast_to_all(JSON.stringify({"message": Message.updateUserAttributes, "data": clients[sender_id]}))
...
func peer_connected(id):
	...
	broadcast_to_all(JSON.stringify({
...
func peer_disconnected(id):
	...
	broadcast_to_all(JSON.stringify(message))
